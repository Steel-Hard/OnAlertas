...
interface Props {
  onSubmit: (data: { title: string; description: string; type: AlertType; location: string }) => void;
}
...
export default function AlertForm({ onSubmit }: Props) {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (!location.trim()) return toast.error("Informe a localização.");

    onSubmit({ title: title.trim(), description: description.trim(), type: type as AlertType, location: location.trim() });
    setTitle("");
    setDescription("");
    setType("");
    setLocation("");
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4 rounded-lg border bg-card p-5">
      <h2 className="text-lg font-semibold text-card-foreground">Novo Alerta</h2>

      <div className="grid gap-4 sm:grid-cols-2">
        <div className="space-y-1.5">
          <Label htmlFor="title">Título *</Label>
          <Input id="title" placeholder="Ex: Buraco na via" value={title} onChange={(e) => setTitle(e.target.value)} />
        </div>
        <div className="space-y-1.5">
      ...
      <div className="space-y-1.5">
        ...
        <Textarea id="desc" placeholder="Detalhes do alerta..." rows={3} value={description} onChange={(e) => setDescription(e.target.value)} />
      </div>
